...
```
pack build tapkindpython1 --path . --builder ghcr.io/vmware-tanzu/function-buildpacks-for-knative/functions-builder:0.3.1 --env BP_FUNCTION=func.main
```

Where `tapkindpython1` is the name of your runnable function image.

Then run via Docker:

```
docker run -it --rm -p 8080:8080 tapkindpython1
```
...
```
tanzu apps workload create tapkindpython1 -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/tapkindpython1-source \
  --type web
```
...
```
tanzu apps workload get tapkindpython1
```
